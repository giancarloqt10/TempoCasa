...
exports.getAgenti = async (req, res) => {
  try {
...
exports.updateAgente = async (req, res) => {
  try {
    const [updated] = await Agente.update(req.body, { where: { id: req.params.id } });
    if (updated) {
...
exports.deleteAgente = async (req, res) => {
  try {
    const deleted = await Agente.destroy({ where: { id: req.params.id } });
    if (deleted) {
      res.status(204).json();
    } else {
      res.status(404).json({ error: 'Agente non trovato' });
